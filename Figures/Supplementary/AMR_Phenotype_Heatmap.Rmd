...
###Check just persistent carriers 
```{r}
setwd("/Users/FEB_ST131/Updated ST131 Code")
load("Apr18_datafinal.Rdata")
subjects_info <- MVdf[MVdf$PERS2FIN == 1,]

info <- subset(subjects_info, select = c("subject_code", "PERS2FIN"))

info <- info[!duplicated(info$subject_code),]

phenot_with_id <- left_join(phenot_short, info, by = "subject_code")

phenot_with_id <- phenot_with_id[!is.na(phenot_with_id$PERS2FIN),] 

phenot_with_id <- phenot_with_id[phenot_with_id$wgs_st131.y ==1,]

```

###Run AMR analysis on just persistent carriers 
...
pers_total <- as.numeric(sum(!is.na(phenot_with_id)))
...
for (i in 1:12){
  ...
  AMR_classshort[5,i] <- AMR_classshort[5, i] / (pers_total * ABsperClass[i]) 
}
...
for(q in 1:ncol(phenot_with_id)){
phenot_with_id[,q] <- ifelse(is.na(phenot_with_id[,q]), "no_info", phenot_with_id[,q])
}

category <- phenot_with_id 
...
if(phenot_with_id$`Doripenem Interpretation`[i] == "R" | phenot_with_id$`Doripenem Interpretation`[i] == "ESBL" | phenot_with_id$`Doripenem Interpretation`[i] == "R*" | phenot_with_id$`Ertapenem Interpretation`[i] == "R" | phenot_with_id$`Ertapenem Interpretation`[i] == "ESBL" | phenot_with_id$`Ertapenem Interpretation`[i] == "R*" | phenot_with_id$`Meropenem Interpretation`[i] == "R" | phenot_with_id$`Meropenem Interpretation`[i] == "R*" | phenot_with_id$`Meropenem Interpretation`[i]== "ESBL"| phenot_with_id$`Imipenem Interpretation`[i] == "R" | phenot_with_id$`Imipenem Interpretation`[i] == "R*" | phenot_with_id$`Imipenem Interpretation`[i] == "ESBL"){
  category$Carb[i]<- 1
}else{
  category$Carb[i] <- 0
}
...
st131_iso_only <- category[category$wgs_st131 ==1,]
