---
title: "2-State_MCMC_Runcode_(By Case - Control)"
...
##load case/control index patient data 
...
collect <- subset(WC_metadata, select = c("subject_code", "case_control", "case_control_fam"))

collect <- collect[!duplicated(collect$subject_code),]
MVdf <- left_join(MVdf, collect, by = "subject_code")

d.short <- subset(MVdf, select = c(subject_code,family_code, date_collected.y, timepoint, overall, index_pt_st131, PERS2FIN, CORES2, case_control.y))

colnames(d.short) <- c("subject_code", "family_code", "date_collected", "timepoint", "ST131POS", "index_pt_st131", "PERS2FIN", "CORES2", "Case_Control")
...
##Subset to "Case" index patients and run MCMC 
```{r}
d.short <- d.short[d.short$Case_Control =="Case",]
...
Cases.indiv.model <- rstan::stan(file="Two_State_MCMC_T5.stan", data=Cases.data, iter=20000, chains = 3, cores=6, control=list(adapt_delta=0.95))

save(Cases.indiv.model, file = 'Cases.indiv.model.Rdata')
...
##Subset to "Control" index patients and run MCMC 
...
##Subset to "control" index patients 
d.short <- d.short[d.short$Case_Control =="Control",]
...
Controls.indiv.model <- rstan::stan(file="Two_State_MCMC_T10.stan", data=Controls.data, iter=20000, chains = 3, cores=6, control=list(adapt_delta=0.95))

save(Controls.indiv.model, file = 'Controls.indiv.model.Rdata')
