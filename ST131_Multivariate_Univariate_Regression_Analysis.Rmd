...
MV1 <- glm(summaryPOS~medicalcond+incontinent+female+ hospital + medevs + age + hometime + subgroup2 + ant_6mnths, data=byindiv, family = "poisson")

summary(MV1)

###carrier status per individual 
MV2 <- glm(Carrier_Stat~medicalcond+incontinent+female+ hospital + medevs + age + hometime + subgroup2 + ant_6mnths, data=byindiv, family = "binomial")
...
obj <- as.data.frame(coef(summary.glm(MV1)))

plotdatameans1 <- obj[-1,1] ##remove intercept 
plotdatasds1 <- obj[-1,2]

plotdataframe1 <- matrix(nrow = 10, ncol= 5, data = NA)
...
plotdataframe1[,5] <- 1:10
plotdataframe1[,4] <-  c("Comorbidity", "Incontinence", "Sex (Female)", "Hospital stay within past year", "Medical devices", "Age", ">30 hours at home per week", "Coresident with persistent carrier", "Persistent carrier", "Antibiotic use within past six months")
...
plotdataframe1 <- plotdataframe1 %>% arrange(factor(Covariate, levels = c("Sex (Female)", "Age", ">30 hours at home per week", "Persistent carrier", "Coresident with persistent carrier", "Comorbidity", "Incontinence", "Medical devices" , "Antibiotic use within past six months", "Hospital stay within past year")))

forester(plotdataframe1[,c(4,6)], plotdataframe1[,1], plotdataframe1[,2], plotdataframe1[,3], ci_sep = " - ", null_line_at = 1, estimate_col_name = "Odds Ratio (95% CI)", file_path = "/Users/lynnperez/Downloads/UPDATE_multivariate_carrier_status.png", x_scale_linear = FALSE, nudge_width = -0.5, font_family = "Arial", xbreaks = c(.1, 1, 5, 10), stripe_colour = "white", point_sizes = 2)
...
```{r}
univalues <- matrix(data = NA, nrow = 10, ncol = 7)

comor <- glm(summaryPOS~medicalcond, data = byindiv, family = "poisson")
...
incont <- glm(summaryPOS~incontinent, data = byindiv, family = "poisson")
...
sex <- glm(summaryPOS~female, data = byindiv, family = "poisson")
...
hosp <- glm(summaryPOS~hospital, data = byindiv, family = "poisson")
...
meddevs <- glm(summaryPOS~medevs, data = byindiv, family = "poisson")
...
age <- glm(summaryPOS~age, data = byindiv, family = "poisson")
...
hometime <- glm(summaryPOS~hometime, data = byindiv, family = "poisson")
...
subgroup <- glm(summaryPOS~subgroup2, data = byindiv, family = "poisson")
summary(subgroup)
obj <- as.data.frame(coef(summary.glm(subgroup)))
univalues[8,1] <- obj[2,1]
univalues[8,2] <- obj[2,2]
univalues[8,7] <- obj[2,4]
univalues[9,1] <- obj[3,1]
univalues[9,2] <- obj[3,2]
univalues[9,7] <- obj[3,4]



ant <- glm(Carrier_Stat~ant_6mnths, data = byindiv, family = "poisson")
obj <- as.data.frame(coef(summary.glm(ant)))
univalues[10,1] <- obj[2,1]
univalues[10,2] <- obj[2,2]
univalues[10,7] <- obj[2,4]
...
Covnames <- as.data.frame(c("Comorbidity", "Incontinence", "Sex (Female)", "Hospital stay within past year", "Medical devices", "Age", ">30 hours at home per week", "Coresident with persistent carrier", "Persistent carrier", "Antibiotic use within past six months"))

colnames(Covnames) <- "Covariate"

univalues <- cbind(univalues, Covnames)

univalues$Index <- seq(1,10, by = 1)
...
plotunivalues$`P value`[9] <- "<0.001"

plotunivalues <- plotunivalues %>% arrange(factor(Covariate, levels = c("Sex (Female)", "Age", ">30 hours at home per week", "Persistent carrier", "Coresident with persistent carrier", "Comorbidity", "Incontinence", "Medical devices" , "Antibiotic use within past six months", "Hospital stay within past year")))


forester(plotunivalues [,c(6,5)], plotunivalues[,1], plotunivalues[,2], plotunivalues[,3], ci_sep = " - ", null_line_at = 1, estimate_col_name = "Odds Ratio (95% CI)", file_path = "/Users/lynnperez/Downloads/UPDATED_univariate_carrier_status.png", x_scale_linear = FALSE, nudge_width = -0.5, font_family = "Arial", xbreaks = c(.1, 1, 5, 10), stripe_colour = "white", point_sizes = 2)
...
forester(plotdataframe2[,c(4,6)], plotdataframe2[,1], plotdataframe2[,2], plotdataframe2[,3], ci_sep = " - ", null_line_at = 1, estimate_col_name = "Odds Ratio (95% CI)", file_path = "/Users/lynnperez/Downloads/UPDATED_multivariate_persister.png", x_scale_linear = FALSE, nudge_width = -0.5, font_family = "Arial", xbreaks = c(.1, 1, 5, 10), xlim = c(.08, 15), stripe_colour = "white", point_sizes = 2)
...
forester(plotunivalues[,c(6,5)], plotunivalues[,1], plotunivalues[,2], plotunivalues[,3], ci_sep = " - ", null_line_at = 1, estimate_col_name = "Odds Ratio (95% CI)", file_path = "/Users/lynnperez/Downloads/UPDATED_univariate_persister.png", nudge_width = -0.5, font_family = "Arial", x_scale_linear=FALSE, xbreaks = c(.1, 1, 5, 10), xlim = c(0.1, 15), stripe_colour = "white", point_sizes = 2)
